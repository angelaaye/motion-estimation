...
for i=2:size(t_laser,1)
    ...
    for n=1:nparticles
        ...
        for beam=1:2         
            ...
            x_curr = [x_particle(n);y_particle(n)] - [ogxmin;ogymin];
            if ~isnan(y_laser(i,j)) && y_laser(i,j)<y_laser_max && y_laser(i,j)>r_min_laser
                laser_step = r_min_laser:ogres:y_laser(i,j);
                for k = 1:length(laser_step)
                    % laser frame --> particle frame --> inertial frame
                    % note laser scans is 10 cm behind origin of robot.
                    % this is the measurement that is obtained if the
                    % particle's position is the true position.
                    point = x_curr + T(1:2, 1:2)*laser_step(k)*[cos(angles(j));sin(angles(j))];%- 0.1*[cos(theta_particle(n));sin(theta_particle(n))];
                    % round to nearest cell
                    grid_point = ceil(point/ogres); 
                    x = grid_point(1);
                    y = grid_point(2);
                    wm = 1.0; % weight multiplier
                    if isnan(x) || isnan(y) || x<1 || y<1 || y>size(ogp,1) || x>size(ogp,2)% outside of grid
                        break
                    % closest cell along laser ray that is occupied (actual
                    % measurement)
                    elseif ogp(y,x) >= 0.7   
                        % compute likelihood (Gaussian with mean at theoretical measurement)
                        wm = exp(-0.5*(y_laser(i,j)-laser_step(k))^2/laser_var)*w_gain;
                        break
                    end
                end
            end
            % update particle weight
            w_particle(n) = w_particle(n) * wm;
