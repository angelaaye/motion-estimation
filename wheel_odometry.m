...
% Formulas for kinematic unicycle model:
% x_dot = v*cos(theta)
% y_dot = v*sin(theta)
% theta_dot = w
% Given v, w, t (in vehicle frame) find x, y, theta (in inertial frame):
% x_dot*h = x(t+h)-x(t) (Forward Euler step)
for i=2:numodom
    theta_dot = omega_odom(i);
    h = t_odom(i) - t_odom(i-1);
    theta_odom(i) = wrapToPi(h*theta_dot + theta_odom(i-1));

    x_dot = cos(theta_odom(i))*v_odom(i);
    y_dot = sin(theta_odom(i))*v_odom(i);
    x_odom(i) = h*x_dot + x_odom(i-1);
    y_odom(i) = h*y_dot + y_odom(i-1);
end
...
for n=1:2
    ...
    x = zeros(1, size(y_laser,1)*size(y_laser,2));
    y = zeros(1, size(y_laser,1)*size(y_laser,2));
    a = theta_interp; % ang
    % loop over laser scans
    for i=1:size(t_laser,1)
        % ------insert your point transformation algorithm here------
        % first patch: only plot if angular velocity is less than 0.1 rad/s
        if abs(omega_interp(i)) < 0.1 
            % Rotation matrix C_iv = C_vi.T:
            H = [cos(a(i)) -sin(a(i)) 0;
                sin(a(i)) cos(a(i)) 0;
                0 0 1];
            for j=1:npoints
                % obtain position in vehicle frame:
                x_curr = y_laser(i,j)*cos_angles(j);
                y_curr = y_laser(i,j)*sin_angles(j);
                % put it in orientation of inertial frame
                inert_pos = H*[x_curr; y_curr; 1];
                % update position in inertial frame
                % second patch: laser scan origin is 10 cm behind robot
                % therefore, remove 10 cm from actual laser measurement
                x(j+npoints*(i-1)) = inert_pos(1) + x_interp(i) - 0.1*cos(a(i));
                y(j+npoints*(i-1)) = inert_pos(2) + y_interp(i) - 0.1*sin(a(i));
            end
        end
